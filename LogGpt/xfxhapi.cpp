...
{
    qDebug()<<"使用模型 讯飞星火 ："<<Config::get_XFXH_MODEL_V().at(Config::get_USER(::EnUser::XFXH_MODEL_ID).toInt());
    qDebug()<<"发送内容为————>"<<str;
    ...
    list<<QString(Config::get_IKS(EnIks::LLM_XFXH).id).toUtf8()
        ...
        <<QString(Config::get_XFXH_MODEL_V().at(Config::get_USER(::EnUser::XFXH_MODEL_ID).toInt())).toUtf8()
