...
{
    float percentage = (mLowerValue - mMinimum) * 1.f / mInterval;
    return handleRect(int(percentage * static_cast<float>(validLength())) + scLeftRightMargin);
}

QRectF RangeSlider::secondHandleRect() const
{
    float percentage = (mUpperValue - mMinimum) * 1.f / mInterval;
    return handleRect(int(percentage * static_cast<float>(validLength())) + scLeftRightMargin + (type.testFlag(LeftHandle) ? scHandleSideLength : 0));
}
...
void RangeSlider::mousePressEvent(QMouseEvent* aEvent)
{
    if(aEvent->buttons() & Qt::LeftButton)
    {
        ...
        firstHandleRectPosValue = static_cast<int>((orientation == Qt::Horizontal) ? firstHandleRect().x() : firstHandleRect().y());
        secondHandleRectPosValue = static_cast<int>((orientation == Qt::Horizontal) ? secondHandleRect().x() : secondHandleRect().y());
...
void RangeSlider::mouseMoveEvent(QMouseEvent* aEvent)
{
    if(aEvent->buttons() & Qt::LeftButton)
    {
        ...
        if(mFirstHandlePressed && type.testFlag(LeftHandle))
        {
            ...
            {
                setLowerValue(int((posValue - mDelta - scLeftRightMargin - scHandleSideLength / 2) * 1.f / float(validLength()) * float(mInterval)) + mMinimum);
            }
            else
            {
        ...
        else if(mSecondHandlePressed && type.testFlag(RightHandle))
        {
            ...
            {
                setUpperValue(int((posValue - mDelta - scLeftRightMargin - scHandleSideLength / 2 - (type.testFlag(DoubleHandles) ? scHandleSideLength : 0)) * 1.f / float(validLength()) * float(mInterval)) + mMinimum);
            }
            else
            {
