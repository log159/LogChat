...
public class Model : MonoBehaviour
{
    ...
    private void InitModelHarmonic()
    {
        ...
        if (!File.Exists(directoryPath + "HARMONICLIST.txt"))
        {
            FileStream fs = File.Create(directoryPath + "HARMONICLIST.txt");//创建文件
            fs.Close();
        }

        if (!File.Exists(directoryPath + "HARMONICCHANGELIST.txt"))
        {
            FileStream fs = File.Create(directoryPath + "HARMONICCHANGELIST.txt");//创建文件
            fs.Close();
        }

        if (!File.Exists(directoryPath + "DRAWINGLIST.txt"))
        {
            FileStream fs = File.Create(directoryPath + "DRAWINGLIST.txt");//创建文件
            fs.Close();
        }

        if (!File.Exists(directoryPath + "DRAWINGCHANGELIST.txt"))
        {
            FileStream fs = File.Create(directoryPath + "DRAWINGCHANGELIST.txt");//创建文件
            fs.Close();
        }

        File.WriteAllText(directoryPath + "HARMONICLIST.txt", strHarmonicListJson);
    ...
    {
        string filePath = Config.DirectoryPath + "HARMONICCHANGELIST.txt";
